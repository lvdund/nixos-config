...
> the machine as a new host (`macbook`) in the existing `nixos-config` flake.
...
> - `flake.nix`: added `nixpkgs-darwin` + `nix-darwin` inputs, `darwinConfigurations.macbook`
> - `darwin/macbook/configuration.nix`: created (optional extras included as comments)
> - `users/vd/macbook.nix`: created (reuses the `nvim`/`fish`/`direnv`/`browser`/`git`
...
>   (`programs.git` + identity settings), imported by `macbook.nix`,
...
>   (`/Users/vd/nixos-config` on the Mac)
>
...
- [ ] Pick a hostname now (e.g. `macbook`) — it becomes the flake attr
  (`darwinConfigurations.macbook`); macOS default hostname has spaces/uppercase,
  which is why we use an explicit `#macbook` attr
...
- [x] Add inputs in `flake.nix` (done — see `flake.nix`):
  ...
  - Tradeoff: single `nixpkgs` for everything (simplest, your Linux hosts stay on
    ...
    with both.
- [x] Add a `darwinConfigurations.macbook` output next to `nixosConfigurations`:
  ```nix
...
- [x] Create `darwin/macbook/configuration.nix` with a minimal start (done, plus optional extras as comments):
  ```nix
  {pkgs, ...}: {
...
- [x] Create `users/vd/macbook.nix` (done, follows the style of `mylaptop.nix`):
  ```nix
  {pkgs, ...}: {
    home.username = "vd";
...
- [x] Extract shared config from existing Linux user files into reusable modules
  ...
  - [x] Reuse `config/kitty` for Kitty terminal (symlink in `macbook.nix`)
  - [x] Parameterize all repo paths: modules take `repoRoot ? "/etc/nixos/nixos-config"`
    ...
    in `flake.nix` (`/Users/vd/nixos-config` on the Mac)
...
- [ ] Document day-2 commands in `notes/`:
  ```sh
  sudo darwin-rebuild switch --flake ~/nixos-config#macbook  # apply
...
| `programs.fish` shell not applied | Set both `programs.fish.enable = true;` (registers `/etc/shells`) and `users.users.vd.shell = pkgs.fish;` |
| Spaces/uppercase in hostname flake attr | Always switch with explicit `--flake .#macbook`, never rely on LocalHostName |
...
sudo darwin-rebuild build --flake ~/nixos-config#macbook
