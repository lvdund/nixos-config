...
BEFORE_X=$(swaymsg -t get_tree | jq '.. | select(.focused? == true).rect.x')

CURRENT_WS=$(swaymsg -t get_workspaces | jq '.[] | select(.focused == true).num')
ALL_WS=$(swaymsg -t get_workspaces | jq '[.[].num] | sort')
MIN_WS=$(echo "$ALL_WS" | jq 'min')
MAX_WS=$(echo "$ALL_WS" | jq 'max')

swaymsg focus $DIRECTION > /dev/null

AFTER_X=$(swaymsg -t get_tree | jq '.. | select(.focused? == true).rect.x')
...
if [ "$IF_WRAP" = true ]; then
    if [ "$WS_DIRECTION" == "prev" ] && [ "$CURRENT_WS" -gt "$MIN_WS" ]; then
        swaymsg workspace $WS_DIRECTION
